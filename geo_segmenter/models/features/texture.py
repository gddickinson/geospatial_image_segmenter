...
class TextureFeatureExtractor(FeatureExtractor):
    ...
    def extract_features(self, data: np.ndarray) -> np.ndarray:
        ...
        """
        try:
            logger.debug("Starting texture feature extraction")
            ...
            # Reduce number of properties and distances for faster computation
            glcm_props = ['contrast', 'homogeneity', 'energy']
            distances = [1]  # Only use distance 1
            angles = [0, np.pi/2]  # Only use horizontal and vertical

            logger.debug("Calculating GLCM features")

            # Calculate local statistics (faster than GLCM)
            for window_size in self.parameters['window_sizes']:
                # Standard deviation
                logger.debug(f"Calculating std dev for window size {window_size}")
                std = ndimage.uniform_filter(gray.astype(float), size=window_size)
                features.append(std)

                # Local range
                logger.debug(f"Calculating range for window size {window_size}")
                footprint = np.ones((window_size, window_size))
                local_min = ndimage.minimum_filter(gray, footprint=footprint)
                local_max = ndimage.maximum_filter(gray, footprint=footprint)
                range_feat = local_max - local_min
                features.append(range_feat)

                # Gradient magnitude (faster than full GLCM)
                logger.debug(f"Calculating gradient for window size {window_size}")
                gradient = ndimage.gaussian_gradient_magnitude(
                    gray.astype(float),
                    sigma=window_size/3
                )
            ...
            logger.debug("Texture feature extraction completed")
            return np.stack(features, axis=0)
